...
DATA_ROOT1='/home/ndzung/DeaKin/code2vec-master_local/code2vec-c-master_local/data'
DATA_ROOT=${CODE2VEC_LOC}/data/${DATASET_NAME}
RAW_DATA_FILE=${DATA_ROOT1}/${DATASET_NAME}.raw.txt
SORTED_DATA_FILE=${DATA_ROOT1}/${DATASET_NAME}.sorted.raw.txt
TRAIN_DATA_FILE=${DATA_ROOT1}/${DATASET_NAME}.train.raw.txt
VAL_DATA_FILE=${DATA_ROOT1}/${DATASET_NAME}.val.raw.txt
TEST_DATA_FILE=${DATA_ROOT1}/${DATASET_NAME}.test.raw.txt

# Ensure previous runs are cleared out...
rm -rf $DATA_ROOT
mkdir -p $DATA_ROOT

echo "Extracting paths from data set...housre"
#echo "PYTHON ${PYTHON}"
#cd /home/dung/Downloads/hpc/test/venv/bin
python /home/ndzung/DeaKin/code2vec-master_local/code2vec-c-master_local/extract-c.py --dir ${SOURCE_DIR} -outdir ${DATA_ROOT} ${SKIP_DECLS} -maxleaves ${MAX_LEAVES} --num_threads ${NUM_THREADS} --clang-path ${CLANG_PATH} --include-path ${SOURCE_DIR} > ${RAW_DATA_FILE} 2>> ${DATA_ROOT}/stderr.txt
