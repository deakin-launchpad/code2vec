...
def normalize_function_name(str):
    with open('function_names.txt', 'a') as function_names:
        function_names.writelines(str + '\n')
    function_names.close()
...
def generate_and_print_paths(function, f=sys.stdout):
    ...
    '''
    df = pd.read_csv('/home/kalrasom/static_analysis/code2vec/test_VDISC.csv')
    outputfile = open('vulnerability_id.csv', 'a')
    index = -1
    functions = df['Function'].values.tolist()
    for func in functions:
        if function.displayname in func:
            index = functions.index(func)
        break
    vuln = ''
    if index > -1:

        vuln = df['Vulnerability'][index]
        outputfile.write(vuln)
    else:
        print("Function skipped - ", function)
    outputfile.close()
    '''
    for s in leaves:
